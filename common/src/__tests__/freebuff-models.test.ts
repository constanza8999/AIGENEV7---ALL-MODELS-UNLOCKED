...
describe('freebuff model availability', () => {
  ...
  test('Fable 5 is a selectable unlimited multimodal model with no data-collection warning', () => {
    ...
    expect(isFreebuffModelId(FREEBUFF_FABLE5_MODEL_ID)).toBe(true)
    expect(isSupportedFreebuffModelId(FREEBUFF_FABLE5_MODEL_ID)).toBe(true)
    expect(isFreebuffPremiumModelId(FREEBUFF_FABLE5_MODEL_ID)).toBe(false)
    expect(isFreebuffTracedModelId(FREEBUFF_FABLE5_MODEL_ID)).toBe(false)
  })
  ...
  test('full-access freebuff models can spawn the gemini-thinker subagent', () => {
    ...
    expect(canFreebuffModelSpawnGeminiThinker(FREEBUFF_FABLE5_MODEL_ID)).toBe(
      true,
    )
