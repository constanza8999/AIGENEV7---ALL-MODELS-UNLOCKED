...
describe.skipIf(!binaryExists)('Freebuff Binary Smoke Tests', () => {
  ...
  test(
    'FREEBUFF_APP_URL runtime env var overrides the baked-in NEXT_PUBLIC_FREEBUFF_APP_URL',
    () => {
      // The baked-in NEXT_PUBLIC_FREEBUFF_APP_URL is whatever the build put in
      // (typically http://localhost:3002 for dev). We do NOT set it in the
      // spawn env — that value is replaced at compile time by `bun --define`
      // and is effectively a hint, not an override. We DO set the runtime
      // override FREEBUFF_APP_URL to a different host:port and assert that
      // the CLI's login error references *that* URL.
      const result = spawnSync(BINARY_PATH, ['login'], {
        encoding: 'utf-8',
        timeout: 10_000,
        env: {
          ...process.env,
          ...
          NEXT_PUBLIC_CB_ENVIRONMENT: 'test',
          NEXT_PUBLIC_CODEBUFF_APP_URL: 'http://127.0.0.1:9',
          // Intentionally NOT setting NEXT_PUBLIC_FREEBUFF_APP_URL: it's
          // baked in and can't be changed at runtime.
          FREEBUFF_APP_URL: 'http://127.0.0.1:8',
          NEXT_PUBLIC_SUPPORT_EMAIL: 'test@example.com',
          NEXT_PUBLIC_POSTHOG_API_KEY: 'test',
          NEXT_PUBLIC_POSTHOG_HOST_URL: 'http://127.0.0.1:9',
          NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY: 'test',
      ...
      // Sanity: we entered the login flow.
      expect(output).toContain('Generating login URL')

      // The runtime override (port 8) is the URL the CLI should have hit.
      // The baked-in value (port 9, :3002, or whatever else the build set)
      // must NOT appear in the error message.
      expect(output).toContain('127.0.0.1:8')
      expect(output).not.toContain('127.0.0.1:9')
      expect(output).not.toContain('localhost:3002')
    },
    TIMEOUT_MS,
  )
