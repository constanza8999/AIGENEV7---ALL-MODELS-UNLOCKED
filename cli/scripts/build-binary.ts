...
async function main() {
  ...
  const defineFlags = [
    ...
    ['process.env.AIGENEV7_LOCAL', `"${process.env.AIGENEV7_LOCAL ?? 'false'}"`],
    ...nextPublicEnvVars,
  ]
  ...
  const buildArgs = [
    ...
    ...(targetInfo.platform === 'win32'
      ? ['--icon', join(repoRoot, 'freebuff', 'icon.ico')]
      : []),
  ...
  if (process.platform === 'win32') {
    // Best-effort unlink the existing --outfile before `bun build --compile`.
    // On Windows, bun's atomic-rename fails with EPERM if the target is still
    // mapped/locked (e.g. a prior build's binary is being held open by an
    // editor, AV, or a running dev session). Failures are non-fatal — we let
    // the runCommand below surface the real error if the unlink couldn't
    // resolve the conflict.
    try {
      if (existsSync(outputFile)) {
        unlinkSync(outputFile)
        logAlways(`[win32] unlinked stale ${outputFile}`)
      }
    } catch (err) {
      const message = err instanceof Error ? err.message : String(err)
      console.error(
        `[win32] could not unlink stale ${outputFile} (${message}); build may fail with EPERM if the binary is still locked.`,
      )
    }
  }
