...
export const useLogo = ({
  ...
  const rawLogoString = useMemo(() => {
    if (maxHeight != null && maxHeight < ASCII_LOGO_LINES) {
      return IS_FREEBUFF ? 'AIGEN7EV' : 'CODEBUFF'
    }
    if (availableWidth >= 55) return LOGO
  ...
  const textBlock = useMemo(() => {
    if (rawLogoString === 'CODEBUFF' || rawLogoString === 'AIGEN7EV') {
  ...
  const component = useMemo(() => {
    ...
    if (rawLogoString === 'CODEBUFF' || rawLogoString === 'AIGEN7EV') {
      const brandName = IS_FREEBUFF ? 'AiGen7Ev' : 'Codebuff'
