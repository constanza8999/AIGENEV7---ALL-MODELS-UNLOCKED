...
export function useFreebuffSession(): UseFreebuffSessionResult {
  ...
  useEffect(() => {
    ...
    // AIGENEV7_LOCAL mode: immediately return an active session without server calls.
    // This lets the CLI skip the landing screen and go straight to chat.
    if (process.env.AIGENEV7_LOCAL === 'true' || process.env.AIGENEV7_LOCAL_MODE === 'true') {
      ...
      setError(null)
      logger.info({}, '[aigenev7] Local mode: session activated without server')
      return
    }
