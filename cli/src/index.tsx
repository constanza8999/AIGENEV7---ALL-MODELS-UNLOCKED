...
async function main(): Promise<void> {
  ...
  const AppWithAsyncAuth = () => {
    ...
    React.useEffect(() => {
      // AIGENEV7_LOCAL mode: skip auth, use local API keys directly.
      // Must set the session BEFORE App renders to avoid a race condition
      // where the auth query fails and shows ServerNotReachableScreen before
      // useFreebuffSession's effect in AuthedSurface can create the local session.
      if (process.env.AIGENEV7_LOCAL === 'true' || process.env.AIGENEV7_LOCAL_MODE === 'true') {
        const localKey = process.env.CODEBUFF_API_KEY || 'local-dev-key'
        setApiClientAuthToken(localKey)
        setHasInvalidCredentials(false)
        setRequireAuth(false)

        // Pre-seed the freebuff session store so AuthedSurface sees
        // an active session immediately (no "Connecting…" flash).
        const model = process.env.AIGENEV7_DEFAULT_MODEL || 'nvidia-llama-3.1-8b'
        useFreebuffModelStore.getState().setSelectedModel(model)
        useFreebuffSessionStore.getState().setSession({
          status: 'active',
          accessTier: 'full' as const,
          instanceId: `local-${crypto.randomUUID().slice(0, 8)}`,
          model,
        ...
        logger.info({}, '[aigenev7] Local mode: auth bypassed, session pre-seeded')
        return
      }
