...
export const App = ({
  ...
  const headerContent = useMemo(() => {
    ...
    return (
      ...
      >
        ...
        >
          {IS_FREEBUFF ? 'AiGen7Ev' : 'Codebuff'} will run commands on your behalf to help you build.
        </text>
        <text
          style={{ wrapMode: 'word', marginBottom: 1, fg: theme.foreground }}
        >
  ...
  if (authQuery.isError) {
    if (authErrorStatusCode === undefined) {
      // Network errors without HTTP status (connection refused, DNS failure, etc.)
      // After retries are exhausted, this is a permanent unreachable state.
      authStatus = authQuery.isFetching ? 'retrying' : 'unreachable'
    } else if (isRetryableStatusCode(authErrorStatusCode)) {
      // Retryable errors (408 timeout, 429 rate limit, 5xx server errors)
      // If still retrying, show 'retrying'; otherwise it's permanently unreachable.
      authStatus = authQuery.isFetching ? 'retrying' : 'unreachable'
  ...
  // Detect local mode (compile-time define replaced at build for AIGENEV7 binary)
  const IS_AIGENEV7_LOCAL =
    typeof process !== 'undefined' &&
    (process.env.AIGENEV7_LOCAL === 'true' || process.env.AIGENEV7_LOCAL_MODE === 'true')
  ...
  // AIGENEV7_LOCAL mode: session was pre-seeded in index.tsx, skip all auth guards
  if (!IS_AIGENEV7_LOCAL) {
    // Show server-not-reachable screen when auth fails with a network/connection error
    if (
      requireAuth !== null &&
      ...
      return (
        <ServerNotReachableScreen
          isLocalMode={false}
          onExit={() => exitFreebuffCleanly()}
        />
      )
    }
...
/**
 * Full-screen error shown when the API server cannot be reached.
 * Displays a clear, actionable message based on whether we're in local mode
 * (AIGENEV7) or normal mode (Codebuff/Freebuff).
 */
interface ServerNotReachableScreenProps {
  isLocalMode: boolean
  onExit: () => void
}

const ServerNotReachableScreen = ({
  isLocalMode,
  onExit,
}: ServerNotReachableScreenProps) => {
  const theme = useTheme()
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {isLocalMode
            ? "Can't connect to local API server"
          ...
          {isLocalMode ? (
            <>
              <text style={{ fg: theme.muted, wrapMode: 'word' }}>
                The local API server is not running on{' '}
                <span fg={theme.primary}>http://localhost:3457</span>.
              </text>
              <text
                style={{ fg: theme.muted, wrapMode: 'word', marginTop: 1 }}
              >
                Start it in another terminal window:
              </text>
              <text
                style={{
                  fg: theme.info,
              ...
              <text style={{ fg: theme.info, wrapMode: 'none', marginTop: 1 }}>
                freebuff\aigenev7-local.bat
              </text>
            </>
          ) : (
            <>
              <text style={{ fg: theme.muted, wrapMode: 'word' }}>
                Could not reach the API server at{' '}
                <span fg={theme.primary}>
                  {typeof process !== 'undefined'
                    ? process.env.NEXT_PUBLIC_CODEBUFF_APP_URL || 'codebuff.com'
                    : 'codebuff.com'}
                </span>
                .
              </text>
              <text
                style={{ fg: theme.muted, wrapMode: 'word', marginTop: 1 }}
              >
                Check your internet connection and try again. Press{' '}
                <span fg={theme.primary}>Ctrl+C</span> to exit, then restart.
              </text>
            </>
          )}
        </box>
